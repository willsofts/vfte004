...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="vfte004" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" :visible="displayPageHeader" />
...
export default {
    ...
    this.$nextTick(async () => {
      ...
      startApplication("vfte004",(data) => {
        ...
        if(data.type=="language") {
          ...
          loadAndMergeLabel("vfte004", (success) => {
            if (success) {
  ...
  methods: {
    ...
    dataSelected(item,action) {
      ...
      if("edit"==action) {
        this.$refs.entryForm.retrieveRecord({roleid: item.roleid});
      } else if("delete"==action) {
        this.$refs.entryForm.startDeleteRecord({nameen: item.nameen, roleid: item.roleid});
      }
    },
    dataInsert(filters) {
      //listen action from search form
